class InverterPricing {
  ...
  async loadExcelData() {
    try {
      ...
      const data = await window.api.readExcel('Inverter');
      if (!data || data.length === 0)
        throw new Error('Inverter için Excel verisi boş veya hatalı.');
  ...
  addToQuote() {
    ...
    const systemName =
      document.getElementById('systemNameInput').value || 'Inverter System';
...
document.addEventListener('DOMContentLoaded', () => {
  window.inverterApp = new InverterPricing();
});

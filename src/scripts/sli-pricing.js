class RectifierPricing {
  ...
  setupEventListeners() {
    ...
    // Panel butonları
    document
      .getElementById('systemCalcBtn')
      .addEventListener('click', () => this.openPanel('systemCalcPanel'));
    document
      .getElementById('batteryCalcBtn')
      .addEventListener('click', () => this.openPanel('batteryCalcPanel'));
    document
      .getElementById('calculateAndAddSystem')
      .addEventListener('click', () => this.calculateAndAddSystem());
  ...
  initializeCalculators() {
    ...
    // Sistem hesaplama - sadece calculateAndAddSystem kullanılıyor
  ...
  async calculateAndAddSystem() {
    try {
      const inputs = this.getSystemCalculationInputs();
      const componentSpecs = this.runCoreCalculations(inputs);

      this.showInfo(
        'Hesaplama Başlatıldı',
        'Bileşenler merkezi konfigürasyondan aranıyor...'
      );
      ...
      if (result.success && result.components.length > 0) {
        this.addComponentsToTable(result.components);
        this.showSuccess(
          'Başarılı!',
          `${result.components.length} adet bileşen sisteme eklendi.`
        );
      } else if (result.success) {
        this.showWarning(
          'Uyarı',
          'Hesaplama başarılı ancak uygun bileşen bulunamadı.'
        );
      } else {
        throw new Error(result.error);
      }

      document.getElementById('systemCalcPanel').classList.remove('active');
    } catch (error) {
  ...
  addComponentsToTable(components) {
    components.forEach((component) => {
      const quantity = 1; // Her bileşen tek olarak eklenir
      const cost = component.cost || 0;
      const totalCost = cost * quantity;
      const margin2Total = totalCost * 1.6;

      const row = this.priceTable.insertRow();

      // Bileşen detaylarını data attribute olarak sakla (gelecekte kullanılabilir)
      row.dataset.componentType = component.type;
  ...
  getSystemCalculationInputs() {
    const fieldMappings = {
      ...
      phase: { id: 'numberOfPhase', name: 'Giriş Fazı', min: 1, max: 3 },
      batteryCapacity: {
        id: 'batteryCapacity',
      ...
      cellCount: { id: 'cells', name: 'Hücre Sayısı', min: 1, max: 200 },
      efficiency: { id: 'efficiency', name: 'Verimlilik', min: 50, max: 100 },
      powerFactor: {
        id: 'powerFactor',
        name: 'Güç Faktörü',
        min: 0.5,
    ...
    // Batarya tipi (string değer)
    inputs.batteryType = document.getElementById('batteryType').value;

    // Diğer alanları kontrol et
    for (const [key, field] of Object.entries(fieldMappings)) {
      const element = document.getElementById(field.id);
      const value =
        key === 'phase' ? parseInt(element.value) : parseFloat(element.value);

      // Boş veya geçersiz değer kontrolü
      if (!element.value.trim() || isNaN(value)) {
        this.validateInput(
          element,
          false,
          `${field.name} alanı boş bırakılamaz`
        );
        hasErrors = true;
        continue;
      }
      ...
      if (value < field.min || value > field.max) {
        this.validateInput(
          element,
          false,
          `${field.name} ${field.min} ile ${field.max} arasında olmalı`
        );
        hasErrors = true;
        continue;
      }
      ...
      inputs[key] = key === 'efficiency' ? value / 100 : value; // Convert efficiency to decimal
    }
  ...
  runCoreCalculations(inputs) {
    // 1. Batarya şarj akımını hesapla
    const chargeCurrentFactor = inputs.batteryType === 'VRLA' ? 0.1 : 0.2;
    const batteryChargeCurrent = inputs.batteryCapacity * chargeCurrentFactor;

    // 2. Rectifier çıkış gücünü hesapla (W)
    const totalFloatVoltage = inputs.floatVoltagePerCell * inputs.cellCount;
    const rectifierOutputPower =
      (inputs.loadCurrent + batteryChargeCurrent) * totalFloatVoltage;

    // 3. Rectifier giriş gücünü hesapla (W)
    const rectifierInputPower = rectifierOutputPower / inputs.efficiency;

    // 4. Transformatör gücünü hesapla (kVA)
    const transformerKVA = rectifierInputPower / 1000 / inputs.powerFactor;

    // 5. Giriş kesicisi akımını hesapla (A)
    const phaseFactor = inputs.phase === 3 ? 1.7 : 1; // Doküman ile uyumlu: 1.7
    const inputCurrent =
      rectifierInputPower / (phaseFactor * inputs.inputVoltage);

    // Bir sonraki adımlar için gerekli olan bileşen özelliklerini döndür
    return {
      transformer: {
        kva: transformerKVA.toFixed(2),
        powerFactor: inputs.powerFactor,
        phase: inputs.phase,
        inputVoltage: inputs.inputVoltage,
      },
      inputBreaker: {
        current: inputCurrent.toFixed(2),
      },
      batteryBreaker: {
        current: (inputs.loadCurrent + batteryChargeCurrent).toFixed(2),
      },
      outputBreaker: {
        current: inputs.loadCurrent.toFixed(2),
      },
      // Güç katı ve elektronik kart için kriterler de buraya eklenebilir.
    };
  }
  ...
  async loadExcelData() {
    try {
      ...
      const data = await window.api.readExcel('Rectifier');
  ...
  addProductToTable() {
    ...
    row.dataset.description = productData.Description || `${selectedProduct}`;
  ...
  addToQuote() {
    ...
    // Veritabanı yerine ana sürece gönder
    window.api.send('add-to-temp-cart', systemPackage);

    this.showSuccess(
      'Sepete Eklendi!',
      `'${systemName}' (x${systemQuantity}) başarıyla sepete eklendi!`
    );
...
document.addEventListener('DOMContentLoaded', () => {
  window.rectifierApp = new RectifierPricing();
});

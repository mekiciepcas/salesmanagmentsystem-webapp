class StabilizerPricing {
  constructor() {
    // Excel yolu
    this.excelPath =
      '//10.0.0.3/c$/Users/epcsql/Desktop/masaustu_uygulama_data/fiyatlandırma excel/Stabilizer.xlsx';
  ...
  async loadExcelData() {
    try {
      ...
      const data = await window.api.readExcel('Stabilizer');
      if (!data || data.length === 0)
        throw new Error('Stabilizer için Excel verisi boş veya hatalı.');
  ...
  addToQuote() {
    ...
    const systemName =
      document.getElementById('systemNameInput').value || 'Stabilizer System';
...
document.addEventListener('DOMContentLoaded', () => {
  window.stabilizerApp = new StabilizerPricing();
});

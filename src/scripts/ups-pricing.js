class UpsPricing {
  constructor() {
    // Excel yolu
    this.excelPath =
      '//10.0.0.3/c$/Users/epcsql/Desktop/masaustu_uygulama_data/fiyatlandırma excel/UPS.xlsx';
  ...
  async loadExcelData() {
    try {
      ...
      const data = await window.api.readExcel('UPS');
      if (!data || data.length === 0)
        throw new Error('UPS için Excel verisi boş veya hatalı.');
  ...
  addProductToTable() {
    ...
    row.dataset.description =
      productData.Description || productData.description || ''; // Büyük/küçükh harf duyarlılığını gider
  ...
  addToQuote() {
    ...
    const systemName =
      document.getElementById('systemNameInput').value || 'UPS System';

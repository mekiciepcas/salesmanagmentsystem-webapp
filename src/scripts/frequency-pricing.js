class FrequencyConverterPricing {
  constructor() {
    // Excel yolu
    this.excelPath =
      '//10.0.0.3/c$/Users/epcsql/Desktop/masaustu_uygulama_data/fiyatlandırma excel/Frequency_Converter.xlsx';
  ...
  async loadExcelData() {
    try {
      ...
      const data = await window.api.readExcel('Frequency_Converter');
      if (!data || data.length === 0)
        throw new Error(
          'Frequency Converter için Excel verisi boş veya hatalı.'
        );
  ...
  addToQuote() {
    ...
    const systemPackage = {
      ...
      items: items,
      productType: 'Frequency Converter',
    };
...
document.addEventListener('DOMContentLoaded', () => {
  window.fcApp = new FrequencyConverterPricing();
});
